...
| [0034](ard-0034-multi-project-workspaces-and-shared-service-layer.md) | 2026-05-27 | Multi-project workspaces + shared service layer (**Proposed**) | Thesis extension from "a repo" to "a set of repos": `boring.workspace.yaml` + `boring up` for ordered multi-repo bring-up, `provides:`/`uses:` shared sidecars on a boring-managed network (restore-once), host-header routing extending ARD-0021, and `extends:` profile composition. Opt-in; single-repo stays default. Targets v1.x. |
| [0035](ard-0035-codex-as-second-agent-tab-in-boring-ui.md) | 2026-05-27 | Codex as a second agent tab in boring-ui | Bounded second per-CLI adapter (Claude + Codex) with an explicit two-harness ceiling. Validates ARD-0026's harness-agnostic abstraction against a real second harness; `ui.agents:` profile field; subscription billing guarded for both (`ANTHROPIC_API_KEY`/`OPENAI_API_KEY` refuse); `~/.codex/` bind-mounted for OAuth persistence. Amends ARD-0029. |
