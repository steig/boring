...
- **Host argv.** boring injects via the devcontainer CLI's `--remote-env KEY=VALUE`, so `GH_TOKEN=<token>` is visible in host process argv (`ps`, `/proc/<pid>/cmdline`) for the duration of `devcontainer up`. This is a pre-existing property of boring's secret channel (every `secret://` rides it the same way); ARD-0044 only changes that a token now rides it *by default* on github.com repos. Tracked for a channel-wide fix (move secrets onto the `BORING_EGRESS_MODE` compose-interpolation pattern, off argv) in steig/boring#41 — deferred to its own change because it must be verified against a live container.
