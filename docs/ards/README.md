...
| [0041](ard-0041-multi-agent-cockpit-on-web-substrate.md) | Accepted | Multi-agent "mission control" cockpit built on the **web** substrate (not a native/Ghostty terminal) — the real gap is concurrent multi-agent panes; the web stack already has the picker + embedded ttyd terminal + diff/preview/undo; native deferred behind a 3-part trigger; from the supacode evaluation; amends ARD-0019 |
| [0042](ard-0042-remote-hosted-boring-access-model.md) | Accepted (phased) | Remote/hosted boring: (a) trusted-share now (proxy token + real TLS, "acts as you"), (b1) team-hosted as the target (per-user identity, multi-user, org-scoped secrets), (b2) public SaaS parked; egress internal-network blocks (#14) are a hard prerequisite; amends ARD-0021/0022/0002/0005 |
| [0043](ard-0043-parallel-worktree-workspaces.md) | Accepted | "Multiple threads + /resume" → built as **parallel worktree workspaces** (the ARD-0041 deep end), not a chat-thread subsystem: a workspace = worktree+branch+full sandbox+backend, registered as a Project, reusing per-slug routing + the #36 dashboard/tabs; single-thread stays the non-engineer default; full isolation per workspace; merge-back deferred to plain git; amends ARD-0022, details ARD-0041 |
